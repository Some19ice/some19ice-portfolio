    ...
    const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                float alpha = smoothstep(0.5, 0.0, distanceFromCenter) * uBaseOpacity;
                vec3 color = mix(vec3(0.55, 0.65, 1.0), vec3(1.0), vTwinkle);
                gl_FragColor = vec4(color, alpha * (0.45 + vTwinkle * 0.55));
            }
        `,
        transparent: true,
...
function animateStars(starData, elapsed) {
    ...
    material.uniforms.uBaseOpacity.value =
    ...
    return (
        <div className="absolute inset-0 z-0">
            <Globe
                ...
                bumpImageUrl={
                    !isMobile ? "//unpkg.com/three-globe/example/img/earth-topology.png" : null
                }
                ...
                hexAltitude={(d) => /** @type {any} */ (d).sumWeight * 0.05}
                ...
                objectAltitude="alt"
                objectLabel={(d) => {
                    const obj = /** @type {any} */ (d)
                    return !obj._isTrail ? obj.name : undefined
                }}
                objectThreeObject={(d) => {
                    const obj = /** @type {any} */ (d)
                    if (obj._isTrail) {
                ...
                onPointClick={(point) => {
                    ...
                    const projectPoint = /** @type {any} */ (point)
                    try {
                        globeEl.current.pointOfView(
                            {
                                lat: projectPoint.lat,
                                lng: projectPoint.lng,
                ...
                onGlobeReady={() => {
                    ...
                    if (globeEl.current) {
                        try {
                            ...
                            /* scene access may not be available immediately */
                        }
                    }

...
const INITIAL_MESSAGES = [
    { type: "system", content: "NASRDA Geospatial Agent v2.0 initialized." },
    {
        type: "ai",
        content:
            "Orbital Command online. Ask me about projects, navigate the globe, or explore the portfolio.",
    },
]
...
function getStaticResponse(command) {
    ...
    if (lower.includes("flood") || lower.includes("cholera") || lower.includes("surveillance")) {
        return {
            text: "**Flood Surveillance** uses geospatial risk visualization for Cross River and other Nigerian regions. I can move the globe there and activate the flood layer for you.",
            action: { lat: 5.8702, lng: 8.5988, altitude: 0.8, layer: "flood" },
        }
    }

    if (lower.includes("abuja") || lower.includes("ngdi") || lower.includes("metadata")) {
        return {
            text: "**NGDI Metadata Portal** is a national-scale geospatial metadata platform built with Next.js, Server Actions, OGC WMS/WFS, and MapLibre. Navigating to Abuja now.",
            action: { lat: 9.0765, lng: 7.3986, altitude: 1.0, layer: null },
        }
    }

    if (lower.includes("lagos") || lower.includes("station") || lower.includes("stock")) {
        return {
            text: "**Station Stock Manager** is a React inventory system with Clerk RBAC, Shadcn UI, and Recharts that helped drive significant revenue growth. Navigating to Lagos now.",
            action: { lat: 6.5244, lng: 3.3792, altitude: 1.1, layer: null },
        }
    }

    if (lower.includes("resume") || lower.includes("cv")) {
        return {
            text: `You can download Yakubu's resume here: [Download Resume](${config.resumeUrl}). You can also reach him at [${config.contactEmail}](mailto:${config.contactEmail}).`,
            action: null,
        }
    }
    ...
    if (lower.includes("project") || lower.includes("portfolio") || lower.includes("work")) {
        return {
            text: `Here are the featured projects I can guide you through:\n${buildProjectSummary()}`,
            action: null,
        }
    }
    ...
    if (lower.includes("contact") || lower.includes("email") || lower.includes("hire")) {
        return {
            text: `Yakubu is available for geospatial systems, full-stack products, and technical consulting. Email [${config.contactEmail}](mailto:${config.contactEmail}) or connect on [LinkedIn](${config.socialMedia.linkedin}).`,
            action: null,
        }
    }

    return {
        text: "Orbital Command is running in static portfolio mode. Ask me about **projects**, the **NGDI portal**, the **Flood dashboard**, or request the **resume**.",
        action: null,
    }
}
...
export default function ChatTerminal({ onCommand }) {
    ...
    const processCommand = useCallback(
        async (cmd) => {
            ...
            try {
                if (!CHAT_ENDPOINT) {
                    const data = getStaticResponse(cmd)
                ...
                if (res.status === 429) {
                    setHistory((prev) => [
                        ...
                        {
                            type: "error",
                            content: data.text || "Too many messages. Please slow down.",
                        },
                    ])
                    setIsLoading(false)
                    return
                ...
                if (!res.ok) {
                    setHistory((prev) => [
                        ...prev,
                        { type: "error", content: data.text || "Failed to reach Orbital Command." },
                    ])
                    setIsLoading(false)
                    return
                }
                ...
                setHistory((prev) => [
                    ...prev,
                    { type: "error", content: "Connection to Orbital Command interrupted." },
                ])
            } finally {
                setIsLoading(false)
            }
